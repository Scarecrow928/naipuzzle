    ...
    <div class="menu-card">
      ...
      <div class="menu-section">
        ...
        <div class="size-selector">
          ...
          <button class="size-btn" :class="{ active: customMode }" @click="enableCustom">Custom</button>
        </div>
        <div v-if="customMode" class="custom-row animate-fade-in">
          <input type="number" class="custom-input" v-model.number="customCols" :min="MIN_GRID_SIZE" :max="MAX_GRID_SIZE" placeholder="Cols" />
          <span class="custom-times">&times;</span>
          <input type="number" class="custom-input" v-model.number="customRows" :min="MIN_GRID_SIZE" :max="MAX_GRID_SIZE" placeholder="Rows" />
        </div>
      ...
      <div class="menu-section">
        ...
        <div class="source-tabs">
          <button class="source-tab" :class="{ active: imageMode === 'preset' }" @click="imageMode = 'preset'">Preset</button>
          <button class="source-tab" :class="{ active: imageMode === 'url' }" @click="imageMode = 'url'">URL</button>
          <button class="source-tab" :class="{ active: imageMode === 'file' }" @click="imageMode = 'file'">Upload</button>
        </div>
        ...
        <div v-if="imageMode === 'file'" class="file-row animate-fade-in">
          <label class="file-label"><span>{{ imageFile ? imageFile.name : 'Choose image...' }}</span><input type="file" accept="image/*" class="file-input-hidden" @change="onFileChange" /></label>
        </div>
      </div>
      ...
      <button class="btn btn-primary btn-lg btn-full" :disabled="loading" @click="handleStart">
        <span v-if="loading" class="spinner"></span>{{ loading ? 'Loading...' : 'Start Game' }}
      </button>
  ...
  <Teleport to="body">
    <div v-if="showCredits" class="credits-overlay" @click.self="showCredits = false">
      <div class="credits-card animate-fade-in-scale">
        <h3 class="credits-title">Links</h3>
...
.credits-link { display: inline-flex; align-items: center; gap: 8px; font-size: 14px; color: var(--color-primary); text-decoration: none; transition: opacity var(--transition-fast); }
